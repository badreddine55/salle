...
export default function CreateEtablissement() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Posting to:", `${apiUrl}/api/etablissements`, formData);

      await axios.post(`${apiUrl}/api/etablissements`, formData, {
        headers: {
      ...
      if (err.response?.status === 401) {
        ...
        setError(`Échec de la création de l'établissement : ${errorMessage}`);
      }
      console.error("Erreur de création de l'établissement :", err);
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <Layout>
      <PageHeader title="Créer un établissement" />
