...
export default function FormateurManagement() {
  ...
  const fetchFormateurs = async () => {
    ...
    try {
      ...
      console.log("Fetching from:", `${apiUrl}/api/formateurs`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const apiUrl = process.env.REACT_APP_API_URL || "http://localhost:5000";
      console.log("Deleting from:", `${apiUrl}/api/formateurs/${id}`);
                  ...
                  {formateurs.map((formateur) => (
                    <tr key={formateur.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <button
                          onClick={() => navigate(`/superadmin/formateurs/edit/${formateur.id}`)}
