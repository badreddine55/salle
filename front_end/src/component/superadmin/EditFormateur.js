...
export default function EditFormateur() {
  ...
  const fetchFormateur = useCallback(async () => {
    setIsLoading(true);
    setErrors({});
    ...
    if (!/^[0-9a-fA-F]{24}$/.test(id)) {
      setErrors({ global: "Format de l'ID du formateur invalide" });
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      console.log("Edit Formateur Response:", response.data); // Debug

      const formateurData = response.data.data || response.data;

      if (!formateurData) {
        throw new Error("Aucune donnée de formateur reçue");
      }

      setFormData({
        matricule: formateurData.matricule || "",
        name: formateurData.name || "",
        email: formateurData.email || "",
        phoneNumber: formateurData.phoneNumber || "",
        address: formateurData.address || "",
        role: formateurData.role || "Formateur",
      });
    } catch (err) {
      ...
      if (err.response?.status === 401) {
        ...
        setErrors({ global: "Formateur non trouvé" });
      } else {
        setErrors({ global: `Échec du chargement du formateur : ${errorMessage}` });
      }
  ...
  useEffect(() => {
    console.log("Formateur ID:", id); // Debug
    fetchFormateur();
  }, [fetchFormateur]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 401) {
        ...
        setErrors({ global: `Échec de la mise à jour du formateur : ${errorMessage}` });
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div className="flex space-x-4">
              <button
                ...
                aria-label="Mettre à jour le formateur"
              >
                {isSubmitting ? (
                  <>
                    <svg
