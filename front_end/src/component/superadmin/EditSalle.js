...
export default function EditSalle() {
  ...
  const fetchSalle = async () => {
    ...
    try {
      ...
      console.log("Fetching from:", `${apiUrl}/api/salles/${id}`);

      const response = await axios.get(`${apiUrl}/api/salles/${id}`, {
        headers: {
      ...
      setFormData({
        ...
        secteur: response.data.data.secteur.join(", ") || "",
      });
    } catch (err) {
      ...
      if (err.response?.status === 401) {
        ...
        setError(`Échec du chargement de la salle : ${errorMessage}`);
      }
      console.error("Erreur de chargement de la salle :", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const apiUrl = process.env.REACT_APP_API_URL || "http://localhost:5000";
      console.log("Updating at:", `${apiUrl}/api/salles/${id}`);

      // Transformer le champ secteur en tableau
      const formattedData = {
        ...formData,
        secteur: formData.secteur.split(",").map((item) => item.trim()),
      };

      await axios.put(`${apiUrl}/api/salles/${id}`, formattedData, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      ...
      if (err.response?.status === 401) {
        ...
        setError(`Échec de la mise à jour de la salle : ${errorMessage}`);
      }
      console.error("Erreur de mise à jour de la salle :", err);
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <Layout>
        <PageHeader title="Modifier la salle" />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white p-6 rounded-lg shadow-sm max-w-lg mx-auto">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">
                Secteurs (séparés par des virgules) <span className="text-[#004AAD]">*</span>
              </label>
              <input
                type="text"
                name="secteur"
